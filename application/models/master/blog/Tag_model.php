<?php
if (!defined('BASEPATH')) exit('No direct script access allowed');

class Tag_model extends ADMIN_Model{

public function __construct()
	{
		parent::__construct();	  	
	}


	/*Tag Datatable*/
	public function get_all_tag()
		{
			if ($this->data['order_by']=='') {
				$orderBy = "ORDER BY t.tag_title  ASC ";
			}else{
				$orderBy = "ORDER BY ".$this->data['order_by']; 
			}

			$query="SELECT t.*,l.title as language,l.title
					FROM blog_tags as t  
					LEFT JOIN languages as l ON l.id=t.tag_language_id
					WHERE t.is_delete = 0 
					AND t.tag_language_id=2
					AND 1=1".$this->data['search_text']." 
					".$orderBy."
					LIMIT ".$this->data['limit_start'].",".$this->data['limit_length']."";
			
			$query_count="SELECT COUNT(t.tag_id) as total 
				FROM blog_tags as t  
				LEFT JOIN languages as l ON l.id=t.tag_language_id
				WHERE t.is_delete=0 
				AND t.tag_language_id=2
				AND 1=1 ".$this->data['search_text']; 
		
			$result = $this->query_result($query);
			$total  = $this->query_result($query_count);			
			if ($result>0 && $total>0) 
				{ 
					return array('total'=>$total[0]->total,"result"=>$result) ;	
				}
			return false;
		}
 
	/*Tag Details*/
	public function get_tag_detail()
		{
			$result  = $this->query("SELECT t.*,t.tag_id as id
				FROM blog_tags as t
				WHERE t.tag_id='".$this->data['id']."' ") ;

			if ( $result->result_id->num_rows > 0 )
			{ 	
				$data=[]; 
				$data['en'] = $result->row_array();		 

				$result1  = $this->query("SELECT t.*,t.tag_id as id
					FROM blog_tags as t
					WHERE t.tag_language_master_id='".$this->data['id']."'
					AND t.tag_language_id=1 ") ;

				if ( $result1->result_id->num_rows > 0 ){
					$data['ar'] = $result1->row_array();		 
				}else{
					$data['ar'] = [];		
				}
				return $data;
			}
	    	else
		   	{
	   	 		return false;
	   		}
		}
  
	/*Tag List*/
	public function get_all_tag_list()
		{
			$result  = $this->query("SELECT t.*
				FROM blog_tags as t
				WHERE t.is_delete=0   
				AND t.tag_language_id=2
				ORDER BY t.tag_title ASC ") ;

			if ( $result->result_id->num_rows > 0 )
			{
				return $result->result();		
			}
	    	else
		   	{
	   	 	return false;
	   		}
		}
		 
	/*Tag Available*/
	public function tag_available($title,$id='')
		{	 
			if (!empty($id)) { 
				$query = "SELECT tag_id
					FROM blog_tags
					WHERE  is_delete=0
					AND tag_title='".$title."'
					AND tag_id<>".$id; 
			}else{
				$query = "SELECT tag_id
					FROM blog_tags
					WHERE is_delete=0
					AND tag_title='".$title."'"; 
			} 
			$result =$this->query($query);
			
			if ( $result->result_id->num_rows > 0)
			{
				return $result->row();	
			}
			return false;
		}

	 
 

}

?>