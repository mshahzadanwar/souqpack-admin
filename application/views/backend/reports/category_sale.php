<div class="container-fluid">
    ...
    <div class="row" style="margin-top: 50px;">
        <div class="col-12">
            ...
            <div class="card">
                <div class="card-body">
                    <h4 class="card-title">Category Sale Report</h4>
                    <div class="table-responsive m-t-40">
                        <table id="example23" class="display nowrap table table-hover table-striped table-bordered" cellspacing="0" width="100%">
                            ...
                            if(isset($_SESSION['ffilter_cats'])){
                                ...
                                $orders = $this->db->query('SELECT  *, OP.price AS op_Pp
                                            FROM orders AS O,
                                            `order_products` AS OP,
                                            `products` AS PR
                                            WHERE O.id = OP.order_id
                                            AND OP.product_id = PR.id
                                            AND PR.category IN ('.$subcat_list.')
                                            AND O.is_deleted  = 0
                                            AND O.status != 7
                                            ORDER BY O.id DESC'
                                          )->result_object();
                            ...
                            foreach($orders as $order){
                                  ...
                                  if(isset($_SESSION['ffilter_cats'])){
                                    $push[] = $order->order_id;
                                    $ppush = array_unique($push);
                                    $total_values += $order->op_Pp;
                                    $orders_count = count($ppush);
                                    //if($order->order_id ==)
                                  }else {
